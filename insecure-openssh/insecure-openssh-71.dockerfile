...
# Install OpenSSH 7.1p1
RUN wget https://cdn.openbsd.org/pub/OpenBSD/OpenSSH/portable/openssh-7.1p1.tar.gz && \
    tar -xzvf openssh-7.1p1.tar.gz -C /tmp && \
    cd /tmp/openssh-7.1p1 && \
    ./configure --with-md5-passwords --with-privsep-path=/var/empty --sysconfdir=/etc/ssh && \
    make && \
    make install

# Cleanup
RUN rm -rf /tmp/openssh-7.1p1 && \
